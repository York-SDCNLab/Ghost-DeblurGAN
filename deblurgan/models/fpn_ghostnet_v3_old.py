...
class FPN(nn.Module):
    ...
    def __init__(self, norm_layer, num_filters=256, pretrained=True):
        ...
        self.lateral4 = GhostModule(960, num_filters, kernel_size=1, bias= False)#nn.Conv2d(960, num_filters, kernel_size=1, bias=False)
...
if __name__== "__main__":
    ...
    model = FPNGhostNetv3(nn.InstanceNorm2d)
    output = model(inputs_)

    inp = inputs_.squeeze().detach().numpy().transpose([2, 1, 0])
